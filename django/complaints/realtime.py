...
def broadcast_complaint_event(complaint_id, event_type, data, visibility='SHARED'):
    ...
    try:
        audiences = {
            ...
            'SHARED': ('user', 'store', 'support'),
            'INTERNAL': ('support',),
        }.get(visibility, ('support',))
        event = {
        ...
        for audience in audiences:
            async_to_sync(channel_layer.group_send)(
                f"complaint_{complaint_id}_{audience}",
                event,
            )
    except Exception:
