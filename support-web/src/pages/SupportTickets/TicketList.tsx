...
export const TicketList = () => {
  ...
  const columns = [
    { key: 'subject', header: 'Help request', render: (ticket: Ticket) => (
    ...
    { key: 'requesterName', header: 'Requested by', render: (ticket: Ticket) => (
    ...
    { key: 'scope', header: 'Area', render: (ticket: Ticket) => ticket.scope === 'GLOBAL' ? 'All cities' : (ticket.cityName || 'City') },
    ...
    { key: 'messageCount', header: 'Messages', render: (ticket: Ticket) => <span className="flex items-center gap-1.5 text-slate-600"><MessageSquare className="h-3.5 w-3.5" />{ticket.messageCount}</span> },
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-5">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><h1 className="text-2xl font-bold tracking-tight text-slate-950">Help Requests</h1><p className="mt-1 text-sm text-slate-500">Questions and problems related to the app, account, verification, payments or technical issues.</p></div>
      ...
      <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-800"><strong>Help requests</strong> are questions about the AARX app or account. Problems between a customer and pharmacy should go to Customer Complaints.</div>
