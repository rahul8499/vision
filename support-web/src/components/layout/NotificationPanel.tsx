...
const routeFor = (type?: unknown, id?: unknown) => type === 'complaint' ? `/complaints/${id}` : type === 'ticket' || type === 'platformsupportticket' ? `/tickets/${id}` : type === 'refund' || type === 'refundrequest' ? `/refunds/${id}` : type === 'safety_report' || type === 'safetyreport' ? `/safety-reports/${id}` : type === 'contact_log' ? `/operations?tab=contacts&follow_up=${id}` : undefined
