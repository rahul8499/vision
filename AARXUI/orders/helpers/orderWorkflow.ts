...
export const getPrimaryAction = (order: SellerOrder): { action: OrderAction; label: string; icon: string; progressAction: string } | null => {
  ...
  if (capabilities.canPack) return { action: 'MARK_PACKED', label: 'Mark as Packed', icon: 'package-variant-closed', progressAction: 'mark_packed' };
  if (capabilities.canMarkReady) return { action: 'READY_PICKUP', label: 'Ready for Pickup', icon: 'store-check-outline', progressAction: 'mark_locked' };
  if (capabilities.canSendDelivery && order.delivery_assignment_status === 'pending') return null;
  if (capabilities.canSendDelivery) return { action: 'OUT_FOR_DELIVERY', label: order.delivery_assignment_status === 'rejected' ? 'Select Other Partner' : 'Select Delivery Partner', icon: 'truck-delivery', progressAction: 'mark_locked' };
  if (capabilities.canComplete) return { action: 'COMPLETE', label: 'Verify OTP & Complete', icon: 'check-decagram-outline', progressAction: 'mark_completed' };
