    ...
    return (
        <View style={{
            ...
            <TouchableOpacity
                ...
                style={{
                    maxWidth: '85%',
                    backgroundColor: isMyMessage ? CHAT_THEME.lightTealCard : CHAT_THEME.whiteCard,
                    ...
                    shadowColor: CHAT_THEME.secondaryText,
            ...
            >
                ...
                {item.reply_to_text && !isDeleted && (
                    <View style={{
                        backgroundColor: isMyMessage ? CHAT_THEME.whiteCard : CHAT_THEME.background,
                        borderLeftWidth: 4,
                        borderLeftColor: isMyMessage ? CHAT_THEME.secondaryTeal : CHAT_THEME.primaryNavy,
                        padding: 6,
                        borderRadius: 10,
                        marginBottom: 4,
                        marginTop: 2
                    }}>
                        <Text style={{ fontSize: 12, color: isMyMessage ? CHAT_THEME.secondaryTeal : CHAT_THEME.primaryNavy, fontWeight: 'bold' }}>
                            {item.reply_to === item.id ? 'Self' : 'Reply'}
                        </Text>
                        <Text style={{ fontSize: 13, color: CHAT_THEME.secondaryText }} numberOfLines={1}>
                            {item.reply_to_text}
                        </Text>
                    </View>
                )}
                ...
                {isDeleted ? (
                    <Text style={{ color: CHAT_THEME.secondaryText, fontStyle: 'italic', fontSize: 14, paddingVertical: 4 }}>
                        <MaterialCommunityIcons name="cancel" size={14} /> This message was deleted
                    </Text>
                ) : (
                    item.text ? (
                        <Text style={{ color: CHAT_THEME.mainText, fontSize: 15, lineHeight: 20, marginBottom: 4, marginTop: 2, fontWeight: '500' }}>
                            {item.text}
                        </Text>
                    ) : null
                )}
...
export default function ChatRoomScreen() {
    ...
    const [headerTitle, setHeaderTitle] = useState<string>(params.userName || params.user_name || params.storeName || params.store_name || '');
    const [partnerContact, setPartnerContact] = useState<string | null>(params.contact || params.mobile || null);
    ...
    useEffect(() => {
        ...
        const handleInitialization = async () => {
            ...
            try {
                ...
                if (res.data.user_name || res.data.store_name) {
                    if (userType === 'store') {
                        setHeaderTitle(res.data.user_name || 'Patient');
                    } else {
                        setHeaderTitle(res.data.store_name || 'Pharmacy');
                    }
                }
                if (res.data.user_contact || res.data.store_contact) {
                    setPartnerContact(userType === 'store' ? res.data.user_contact : res.data.store_contact);
                }
    ...
    const initiateLazyThread = async (messageText?: string) => {
        ...
        try {
            ...
            // Critical: Update the URL parameters in place without triggering screen unmount/blink
    ...
    const handleVoiceCall = () => {
        if (partnerContact) {
            Linking.openURL(`tel:${partnerContact}`);
        } else {
            Alert.alert("Voice Call", "Direct phone call will connect to verified order contact once available.");
        }
    };
    ...
    return (
        <>
            <StatusBar barStyle="light-content" backgroundColor={CHAT_THEME.primaryNavy} />
            <SafeAreaView style={{ flex: 1, backgroundColor: CHAT_THEME.primaryNavy }}>

                {/* HEADER */}
                <View style={{ zIndex: 50, overflow: 'hidden' }}>
                    <LinearGradient
                        colors={[CHAT_THEME.primaryNavy, '#184C75', CHAT_THEME.secondaryTeal]}
                        start={{ x: 0, y: 0 }}
                        end={{ x: 1, y: 0 }}
                        style={{
                            flexDirection: 'row',
                    ...
                    >
                        <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                            <TouchableOpacity onPress={() => router.back()} style={{ width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(255,255,255,0.12)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center' }}>
                                <Ionicons name="arrow-back" size={22} color="#ffffff" />
                            </TouchableOpacity>
                            <View style={{ width: 42, height: 42, borderRadius: 16, backgroundColor: 'rgba(15,139,141,0.25)', borderWidth: 1, borderColor: CHAT_THEME.borderTeal, alignItems: 'center', justifyContent: 'center', marginLeft: 10, overflow: 'hidden' }}>
                                <MaterialCommunityIcons name={userType === 'user' ? 'storefront-outline' : 'account-outline'} size={22} color="#ffffff" />
                                <View style={{ position: 'absolute', right: -1, bottom: -1, width: 12, height: 12, borderRadius: 6, backgroundColor: otherStatus.is_online ? CHAT_THEME.successGreen : CHAT_THEME.secondaryText, borderWidth: 2, borderColor: CHAT_THEME.primaryNavy }} />
                            </View>
                            ...
                            <View style={{ marginLeft: 12, flex: 1, justifyContent: 'center' }}>
                                ...
                                <Text style={{ color: isOtherTyping ? CHAT_THEME.lightTealCard : 'rgba(255,255,255,0.85)', fontSize: 11, marginTop: 2, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 1.2 }}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', marginRight: 4 }}>
                            <TouchableOpacity
                                ...
                                style={{ marginLeft: 10, width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(255,255,255,0.12)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center' }}
                            >
                    ...
                    <View style={{ flex: 1, backgroundColor: '#f1f5f9' }}>
                        ...
                        {replyingTo && (
                            <View style={{
                                ...
                                borderLeftWidth: 4,
                                borderLeftColor: CHAT_THEME.secondaryTeal,
                                padding: 10,
                                marginHorizontal: 12,
                                marginTop: 8,
                                borderRadius: 18,
                                ...
                                <View style={{ flex: 1 }}>
                                    <Text style={{ color: CHAT_THEME.secondaryTeal, fontWeight: '900', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1 }}>
                                        Replying to {replyingTo.sender_type === userType ? 'yourself' : storeName}
                                    </Text>
                                    <Text style={{ color: CHAT_THEME.secondaryText, fontSize: 13, fontWeight: '600' }} numberOfLines={1}>
                                        {replyingTo.text || 'Media'}
                                    </Text>
                                </View>
                                <TouchableOpacity onPress={() => setReplyingTo(null)}>
                                    <Ionicons name="close-circle" size={24} color={CHAT_THEME.secondaryText} />
                                </TouchableOpacity>
                            </View>
                        )}
                        ...
                        {editingMessage && (
                            <View style={{
                                ...
                                <View style={{ flex: 1 }}>
                                    <Text style={{ color: CHAT_THEME.secondaryTeal, fontWeight: '900', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1 }}>Edit message</Text>
                                    <Text style={{ color: CHAT_THEME.secondaryText, fontSize: 13, fontWeight: '600' }} numberOfLines={1}>{editingMessage.text}</Text>
                                </View>
                        ...
                        {isChatLocked ? (
                            <View style={{
                                ...
                                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
                                    <Ionicons name="lock-closed" size={16} color={CHAT_THEME.secondaryText} style={{ marginRight: 6 }} />
                                    <Text style={{ fontSize: 13, fontWeight: '800', color: CHAT_THEME.secondaryText, textTransform: 'uppercase', letterSpacing: 1 }}>
                                        {orderStatus === 'completed' ? 'COMPLETED' : 'CLOSED'}
                                    </Text>
                                </View>
                                <Text style={{ fontSize: 15, color: CHAT_THEME.mainText, fontWeight: '600', textAlign: 'center' }}>
                                    This chat is no longer active
                                </Text>
                                <Text style={{ fontSize: 13, color: CHAT_THEME.secondaryText, marginTop: 4, textAlign: 'center' }}>
                            ...
                            <View style={{
                                ...
                                {isRecording ? (
                                    <View style={{
                                        ...
                                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                            <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: CHAT_THEME.errorRed, marginRight: 8, opacity: recordDuration % 2 === 0 ? 1 : 0.5 }} />
                                            <Text style={{ fontSize: 16, color: CHAT_THEME.errorRed, fontWeight: '500' }}>
                                                {formatRecordTime(recordDuration)}
                                            </Text>
                                        </View>
                                        <TouchableOpacity onPress={discardRecording}>
                                            <Text style={{ color: CHAT_THEME.secondaryTeal, fontSize: 15, fontWeight: '800' }}>Cancel</Text>
                                        </TouchableOpacity>
                                    </View>
                                ) : previewUri ? (
                                    <View style={{
                                        ...
                                        <TouchableOpacity onPress={discardRecording} style={{ marginHorizontal: 8 }}>
                                            <Ionicons name="trash-outline" size={24} color={CHAT_THEME.errorRed} />
                                        </TouchableOpacity>
                                    ...
                                    <View style={{
                                        ...
                                        <TouchableOpacity style={{ marginHorizontal: 8, marginBottom: 4, justifyContent: 'center' }}>
                                            <MaterialCommunityIcons name="emoticon-outline" size={24} color={CHAT_THEME.secondaryTeal} />
                                        </TouchableOpacity>

                                        <TextInput
                                            style={{
                                                flex: 1,
                                                color: CHAT_THEME.mainText,
                                                fontSize: 16,
                                                fontWeight: '600',
                                                maxHeight: 100,
                                                paddingTop: 4,
                                            ...
                                            placeholder="Message"
                                            placeholderTextColor={CHAT_THEME.secondaryText}
                                            multiline={true}
                                            onFocus={() => setTimeout(() => flatListRef.current?.scrollToOffset({ offset: 0, animated: true }), 100)}
                                        />
                                        ...
                                        >
                                            <MaterialCommunityIcons name="paperclip" size={24} color={CHAT_THEME.secondaryTeal} />
                                        </TouchableOpacity>
                                        <TouchableOpacity
                                            onPress={takePhoto}
                                            style={{ marginRight: 8, marginBottom: 4, justifyContent: 'center' }}
                                        >
                                            <Ionicons name="camera-outline" size={24} color={CHAT_THEME.secondaryTeal} />
                                        </TouchableOpacity>
                                    </View>
                                )}
                                ...
                                <TouchableOpacity
                                    ...
                                    style={{
                                        ...
                                        height: 48,
                                        borderRadius: 24,
                                        backgroundColor: isRecording ? CHAT_THEME.errorRed : CHAT_THEME.primaryNavy,
                                        alignItems: 'center',
                                        justifyContent: 'center',
                                        borderWidth: 1,
                                        borderColor: isRecording ? CHAT_THEME.errorRed : CHAT_THEME.primaryNavy,
                                    }}
                                >
                ...
                >
                    <Pressable
                        style={{ flex: 1, backgroundColor: 'rgba(16,42,67,0.65)', justifyContent: 'center', alignItems: 'center', paddingHorizontal: 24 }}
                        onPress={() => setSelectedMessage(null)}
                    >
                        <View style={{ backgroundColor: CHAT_THEME.whiteCard, borderRadius: 28, width: '100%', maxWidth: 360, paddingVertical: 10, elevation: 12, overflow: 'hidden', borderWidth: 1, borderColor: CHAT_THEME.borderTeal }}>
                            <LinearGradient
                            ...
                            {!selectedMessage?.is_deleted_for_everyone && (
                                ...
                                >
                                    <MaterialCommunityIcons name="reply" size={20} color={CHAT_THEME.secondaryTeal} />
                                    <Text style={{ marginLeft: 16, fontSize: 15, color: CHAT_THEME.mainText, fontWeight: '800' }}>Reply</Text>
                                </TouchableOpacity>
                            )}
                            ...
                            {selectedMessage?.sender_type === userType && !selectedMessage.audio && !selectedMessage.is_deleted_for_everyone && isWithin30Mins(selectedMessage.created_at) && (
                                ...
                                >
                                    <Ionicons name="pencil" size={20} color={CHAT_THEME.secondaryTeal} />
                                    <Text style={{ marginLeft: 16, fontSize: 15, color: CHAT_THEME.mainText, fontWeight: '800' }}>Edit</Text>
                                </TouchableOpacity>
                            )}
                            ...
                            {selectedMessage?.sender_type === userType && !selectedMessage.is_deleted_for_everyone && isWithin30Mins(selectedMessage.created_at) && (
                                ...
                                >
                                    <Ionicons name="trash" size={20} color={CHAT_THEME.errorRed} />
                                    <Text style={{ marginLeft: 16, fontSize: 15, color: CHAT_THEME.errorRed, fontWeight: '800' }}>Delete for everyone</Text>
                                </TouchableOpacity>
                            )}
                            ...
                            >
                                <Ionicons name="trash-outline" size={20} color={CHAT_THEME.secondaryText} />
                                <Text style={{ marginLeft: 16, fontSize: 15, color: CHAT_THEME.mainText, fontWeight: '800' }}>Delete for me</Text>
                            </TouchableOpacity>
                            ...
                            <TouchableOpacity
                                ...
                                onPress={() => setSelectedMessage(null)}
                            >
                                <Ionicons name="close" size={20} color={CHAT_THEME.secondaryText} />
                                <Text style={{ marginLeft: 16, fontSize: 15, color: CHAT_THEME.secondaryText, fontWeight: '800' }}>Cancel</Text>
                            </TouchableOpacity>
                        </View>
                    </Pressable>
                </Modal>
            ...
            {mediaLoading && (
                <View style={{
                    ...
                    <View style={{
                        ...
                        <ActivityIndicator size="large" color={CHAT_THEME.secondaryTeal} />
                        <Text style={{ marginTop: 15, fontSize: 14, fontWeight: '900', color: CHAT_THEME.mainText, textTransform: 'uppercase', letterSpacing: 2 }}>Sending...</Text>
                    </View>
                </View>
            )}
        </>
